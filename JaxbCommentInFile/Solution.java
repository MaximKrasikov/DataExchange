    ...
    public static String toXmlWithComment(Object obj, String tagName, String comment) {
        ...
        try {
            ...
            if(root.hasChildNodes()) {
                ...
                queue.offer(root.getChildNodes());//добавляет элемент в очередь
                while (!queue.isEmpty()) {
                    NodeList list = queue.poll();// извлекает и удаляет голову очереди
                    for (int i = 0; i<list.getLength(); i++){
                        Node node = list.item(i);// возвращает индекс
